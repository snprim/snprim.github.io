...
$$
\begin{align}
R(x,\theta) &= \frac{p_\theta(x)}{p_{\hat{\theta}}(x)} = p_\theta(x) = p(|x-\theta|),
\end{align}
$$
...
\end{align}
$$
where $P$ is the cumulative distribution function for standard Normal distribution. The above derivation works for $n=1$. When $n>1$, the sufficient statistics (sample mean) and the sample variance can be used. 
